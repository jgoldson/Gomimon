#!/usr/bin/env python3
"""Compile ImageGen frames: uv run --with pillow python scripts/build-bubble-gomi.py."""
from pathlib import Path
import json
from PIL import Image

ROOT = Path(__file__).resolve().parents[1]
ART = ROOT / "output/imagegen/bubble-gomi-production"
...
scale = 36 / max(frame.height for row in cells for frame in row)
states = {
    # Avoid the concept's oversized second idle pose and standing sleep pose.
    "idle": (0, [0, 2, 0, 3], [650, 250, 650, 150]),
    "eat": (1, [0, 1, 2, 3], [200, 250, 350, 400]),
    "celebrate": (2, [0, 1, 2, 3], [200, 200, 250, 250]),
    "sleep": (3, [0, 2, 3, 2], [650, 650, 650, 650]),
}
...
for row_number, (state, (row, order, timings)) in enumerate(states.items()):
    ...
    target = DEST / f"bubble-gomi_{state}.gif"
    ...
    if state == "idle":
        frames[0].save(DEST / "bubble-gomi_static.gif", transparency=0)
        frames[0].save(ROOT / "website/assets/bubble-gomi.png", transparency=0)
        (ROOT / "website/assets/bubble-gomi_idle.gif").write_bytes(target.read_bytes())
